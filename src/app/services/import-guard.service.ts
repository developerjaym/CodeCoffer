...
export class ImportGuardService {
  ...
  canActivate(route: ActivatedRouteSnapshot): Observable<boolean> {
    ...
    return this.http
      .get<ChatMessage[]>(environment.serverUrl + 'conversations/' + conversationId + '?startingIndex=' + 0)
      .pipe(
        map(messages => messages[0]),
        tap(
          message => this.remoteImportService.displayImport(message.message.content),
